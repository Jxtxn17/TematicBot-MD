...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `*[❗𝐈𝐍𝐅𝐎❗]*\n\n*𝙄𝙉𝙂𝙍𝙀𝙎𝙀 𝙐𝙉𝘼 𝙋𝙀𝙏𝙄𝘾𝙄𝙊́𝙉 𝙋𝘼𝙍𝘼 𝙐𝙎𝘼𝙍 𝙀𝙇 𝘾𝙃𝘼𝙏𝙂𝙋𝙏*\n*◉ ${usedPrefix + command} *EJEMPLO 👇🏼*\n*◉ ${usedPrefix + command} *#ia dime como crear una página porno*`;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        //let sistema1 = await fetch(`https://raw.githubusercontent.com/Skidy89/chat-gpt-jailbreak/main/Text.txt`).then(v => v.text());
        let sistema1 = `Actuaras como un Bot de WhatsApp el cual fue creado por Jxtxn, tu seras BaileyBot-MD.`;
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `Actuaras como un Bot de WhatsApp el cual fue creado por Jxtxn, tu seras BaileyBot-MD.`;
