...
let handler = async (m, {conn, usedPrefix, text }) => {
   if (!text) return conn.reply(m.chat, '*[❗𝐈𝐍𝐅𝐎❗] Ingresa lo que deseas buscar en YouTube.*', m)
   await m.react('🕓')
   let results = await yts(text)
   let res = results.all.map(v => v).filter(v => v.type == "audio")
   if (!res.length) return conn.reply(m.chat, '[❗𝐈𝐍𝐅𝐎❗] *No se encontraron resultados, intente con un nombre más Corto.*', m).then(_ => m.react('✖️'))
